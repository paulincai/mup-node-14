...
describe('module - mongo', function() {
  ...
  describe('logs', () => {
    it('should pull logs from "meteor" vm', async () => {
      ...
      sh.exec('mupix setup');
      const out = sh.exec('mupix mongo logs');
  ...
  describe('setup', () => {
    it('should setup mongodb on "mongo" vm', async () => {
      ...
      const out = sh.exec('mupix mongo setup');
  ...
  describe('start', () => {
    it('should start mongodb on "mongo" vm', async () => {
      ...
      sh.cd(path.resolve(os.tmpdir(), 'tests/project-1'));
      sh.exec('mupix docker setup && mupix mongo setup');

      const out = sh.exec('mupix mongo start');
      expect(out.code).to.be.equal(0);
    ...
    it('should allow configuring db name', async () => {
      sh.cd(path.resolve(os.tmpdir(), 'tests/project-1'));
      const out = sh.exec('mup --config mupix.db-name.js validate --show');
  ...
  describe('stop', () => {
    it('should stop mongodb on "mongo" vm', async () => {
      ...
      sh.cd(path.resolve(os.tmpdir(), 'tests/project-1'));
      sh.exec('mupix docker setup && mupix mongo setup && mupix mongo start');

      const out = sh.exec('mupix mongo stop');
