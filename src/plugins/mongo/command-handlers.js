...
const log = debug('mupix:module:mongo');

export function logs(api) {
  log('exec => mupix mongo logs');
...
export function setup(api) {
  log('exec => mupix mongo setup');
  const mongoConfig = api.getConfig().mongo;

  if (!mongoConfig) {
    // could happen when running "mupix mongo setup"
    console.log(
  ...
  if (meteorSessions.length !== 1) {
    console.log(
      'To use mupix built-in mongodb setup, you should have only one meteor app server. To have more app servers, use an external mongodb setup'
    );

    return;
  } else if (mongoSessions[0]._host !== meteorSessions[0]._host) {
    console.log(
      'To use mupix built-in mongodb setup, you should have both meteor app and mongodb on the same server'
    );
