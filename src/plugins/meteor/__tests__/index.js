...
describe('module - meteor', function() {
  ...
  describe('setup', () => {
    it('should setup environment on "meteor" vm', async () => {
      ...
      const out = sh.exec('mupix meteor setup');
  ...
  describe('push', () => {
    it('should push meteor app bundle to "meteor" vm', async () => {
      ...
      sh.exec('mupix docker setup');
      sh.exec('mupix meteor setup');

      const out = sh.exec('mupix meteor push --cached-build');
      assert.equal(out.code, 0);
    ...
    it('should handle env vars with space during Prepare Bundle', async () => {
      ...
      const out = sh.exec('mupix --config mupix.env-with-space.js meteor push --cached-build');
      assert.equal(out.code, 0);
