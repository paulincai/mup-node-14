...
describe('module - meteor swarm', function() {
  ...
  this.afterAll(() => {
    cdSwarmProject();
    sh.exec('mupix docker destroy-cluster');
  });

  describe('envconfig', () => {
    it('should not run when swarm is enabled', async () => {
      cdSwarmProject();
      sh.exec('mupix setup && mupix meteor push --cached-build');
      const out = sh.exec('mupix meteor envconfig');
  ...
  describe('start', () => {
    it('should create service', async () => {
      cdSwarmProject();
      sh.exec(
        'mupix setup && mupix meteor push --cached-build'
      );

      const out = sh.exec('mupix meteor start');
  ...
  describe('stop', () => {
    it('should remove service', async () => {
      ...
      cdSwarmProject();
      sh.exec(
        'mupix setup && mupix meteor deploy --cached-build'
      );
